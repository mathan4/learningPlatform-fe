...
const MentorDashboard = () => {
  ...
  console.log(lessons)
  // Refresh function to refetch data on actions like accept or schedule
  const fetchData = async () => {
    try {
      ...
      const earningsRes = await lessonServices.getMentorEarnings();
  ...
  const upcomingLessons = lessons.filter(lesson => lesson?.status === "accepted" || lesson?.status === "scheduled");
            ...
            {lessons.map((lesson) => {
              return (
                ...
                >
                  ...
                  <p className="text-gray-300">
                    Start: {getFormattedDateTime(lesson.startTime)}
                  </p>
                  <p className="text-gray-300">
                    End: {getFormattedDateTime(lesson.endTime)}
                  </p>
                  ...
                  {lesson.status === "pending" && (
                    ...
                    >
                      {scheduling ? "Accepting & Scheduling..." : "Accept & Schedule Lesson"}
                    </button>
                  )}

                  {lesson.status === "accepted" && !lesson.zoomMeetingUrl && (
                    <button
                      onClick={() => scheduleMeeting(lesson._id)}
                      disabled={scheduling}
                      className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-800 disabled:cursor-not-allowed text-white py-2 rounded transition-colors"
                    >
                      {scheduling ? "Scheduling..." : "Schedule Zoom Meeting"}
                    </button>
                  )}

                  {(lesson.status === "scheduled" || lesson.zoomMeetingUrl) && (
                    <a
